...
public interface CarRepo extends JpaRepository<Car,String> {
    ...
    @Query(value = "select * from Car where mileage % 5000 = 0 or maintainStatus='Yes'", nativeQuery = true)
    List<Car> checkNeedToMaintain();

    @Query(value = "SELECT c FROM Car c WHERE (:transmissionType is null OR c.transmissionType=:transmissionType) AND (:brand is null OR c.brand=:brand) AND (:fuelType is null OR c.fuelType=:fuelType) AND (:type is null OR c.type=:type) AND (:numberOfPassengers = 0 OR c.numberOfPassengers=:numberOfPassengers)")
    ...
    @Query(value = "select * from Car where number not in (select car_number from Reservation where ? between pick_up_date and return_date)", nativeQuery = true)
    List<Car> checkTodayAvailableCars(Date todayDate);
}
